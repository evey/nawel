...
function Test-Prerequisites {
    ...
    try {
        ...
        Write-Success "sqlite3 is installed"
    }
    catch {
        Write-Error-Message "sqlite3 is not installed"
    ...
    if (-not (Test-Path $OldSql)) {
        Write-Error-Message "Old database file not found: $OldSql"
    ...
    if (-not (Test-Path $DbFile)) {
        Write-Warning-Message "Database file not found: $DbFile"
...
function Copy-Avatars {
    ...
    if (-not (Test-Path $OldAvatarDir)) {
        Write-Warning-Message "Old avatar directory not found: $OldAvatarDir"
...
function Invoke-Verification {
    ...
    if ([int]$orphaned -gt 0) {
        Write-Warning-Message "$orphaned group gifts have no participations"
    }
    else {
...
function Show-Help {
    ...
    Write-Host ""
    Write-Host "This script migrates data from old\nironico_nawel.sql to the new database."
...
try {
    ...
    Write-Host "1. Restore the backup if needed: Copy-Item $BackupFile nawel.db -Force"
    Write-Host "2. Check the prerequisites"
    Write-Host "3. See Migrations\MIGRATION_GUIDE.md for troubleshooting"
    exit 1
}
