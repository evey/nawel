...
function Count-Lines {
    param($Path, $Pattern)
    $files = Get-ChildItem -Path $Path -Include $Pattern -Recurse -File | Where-Object { $_.FullName -notlike "*node_modules*" -and $_.FullName -notlike "*bin*" -and $_.FullName -notlike "*obj*" }
    $totalLines = 0
    $totalFiles = 0
    $codeLines = 0
    $commentLines = 0
...
$cssFiles = Count-Lines -Path "frontend\nawel-app\src" -Pattern "*.less","*.css","*.module.less"
...
$configFiles = Get-ChildItem -Path "." -Include "*.json","*.csproj","package.json","tsconfig.json" -Recurse -File | Where-Object { $_.FullName -notlike "*node_modules*" -and $_.FullName -notlike "*bin*" -and $_.FullName -notlike "*obj*" }
Write-Host "  Config Files: $($configFiles.Count) files" -ForegroundColor Yellow

# Project Structure
Write-Host "`n`n📁 PROJECT STRUCTURE" -ForegroundColor Green
Write-Host "─────────────────────────────────────" -ForegroundColor Gray
...
$middleware = (Get-ChildItem -Path "backend\Nawel.Api\Middleware" -Filter "*.cs" -File -ErrorAction SilentlyContinue).Count
...
Write-Host "    DTOs:         $dtos" -ForegroundColor White
Write-Host "    Middleware:   $middleware" -ForegroundColor White
...
$hooks = (Get-ChildItem -Path "frontend\nawel-app\src\hooks" -Filter "*.ts" -File -ErrorAction SilentlyContinue).Count
...
Write-Host "    Contexts:     $contexts" -ForegroundColor White
Write-Host "    Hooks:        $hooks" -ForegroundColor White
...
Write-Host "`n`n✨ CODE QUALITY METRICS" -ForegroundColor Green
Write-Host "─────────────────────────────────────" -ForegroundColor Gray

$commentRatio = [math]::Round((($tsxFiles.Comments + $tsFiles.Comments + $csFiles.Comments) / ($tsxFiles.Code + $tsFiles.Code + $csFiles.Code) * 100), 1)
Write-Host "  Comment Ratio:       $commentRatio%" -ForegroundColor White

$testCodeRatio = [math]::Round(($testTotal / ($frontendTotal + $csFiles.Total) * 100), 1)
Write-Host "  Test Coverage:       $testCodeRatio%" -ForegroundColor White

$avgFileSize = [math]::Round($totalCode / $totalFiles, 0)
Write-Host "  Avg File Size:       $avgFileSize lines/file" -ForegroundColor White
